...
global_settings_dirp = pk.join(getenv("APPDATA"), "ShadowCraft")
...
if __name__ == '__main__':
    ...
    if "/forever" in argv:
        ...
        g2c2.gui_main()
        start_time.append(pk.time())
        g2c2.record_fx(f"%s 外部时间计算：第一阶段用时 %.2fs" % (ShadowCraft.__name__, start_time[1] - start_time[0]))
        g2c2.record_fx(f"%s 外部时间计算：第二阶段用时 %.2fs" % (ShadowCraft.__name__, start_time[2] - start_time[1]))
        g2c2.mainloop()
        g2c2.record_fx("正在退出. . . . . . ")
        g2c2.save()
